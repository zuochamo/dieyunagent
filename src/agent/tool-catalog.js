...
(function (root, factory) {
  ...
  root.DieyunToolCatalog = exported;
  ...
  const BROWSER_TOOLS = [
    ...
    },
    {
      ...
      function: {
        ...
        description:
          '获取当前浏览器页面的可交互元素列表（ref）与文本摘要。点击/输入前必须先 snapshot 拿到 ref；页面跳转或 DOM 大变后需重新 snapshot。可选 delayMs 等待 SPA 渲染。',
        parameters: {
          type: 'object',
          properties: {
    ...
    },
    {
      ...
      function: {
        ...
        description:
          '获取页面可访问性（a11y）树：角色、名称、状态。DOM 混乱或 snapshot 找不到元素时可用；不返回可点击 ref，需配合 browser_snapshot 或 selector。',
        parameters: {
          type: 'object',
          properties: {
    ...
    },
    {
      ...
      function: {
        name: 'browser_click',
        description:
          '在浏览器页面点击元素。必须先 browser_snapshot 拿到 ref（如 ref-0），再传入 ref 点击；每次页面变化后需重新 snapshot。不要用 host_open_url 代替 browser_navigate。可选 button=right 或 clickCount=2 做右键/双击。',
        parameters: {
          type: 'object',
          properties: {
    ...
    },
    {
      ...
      function: {
        ...
        description: '双击元素（打开文件、选中词、表格编辑等）。需 ref 或 selector。',
        parameters: {
          type: 'object',
          properties: {
            ref: { type: 'string' },
    ...
    },
    {
      ...
      function: {
        ...
        description: '右键点击元素（上下文菜单）。需 ref 或 selector。',
        parameters: {
          type: 'object',
          properties: {
            ref: { type: 'string' },
    ...
    },
    {
      ...
      function: {
        name: 'browser_type',
        description: '向输入框输入文本（可追加，默认不清空）。',
        parameters: {
          type: 'object',
          properties: {
            ref: { type: 'string' },
    ...
    },
    {
      ...
      function: {
        ...
        description:
          '截取浏览器页面或单个元素截图（PNG base64）。可传 ref/selector 仅截元素区域。给 filePath 时会同时把 PNG 落盘作为验收留证（返回 path）。',
        parameters: {
          type: 'object',
          properties: {
    ...
    },
    {
      ...
      function: {
        ...
        description:
          '在当前页面执行 JavaScript 并返回结果（JSON 文本，超长会截断）。用于读取页面数据、调用页面内 API、或 snapshot/ref 覆盖不到的精确操作。高危：可读写页面与同源数据（含登录态），仅在确有必要时使用，不要执行来源不明的脚本。',
        parameters: {
          type: 'object',
          properties: {
            script: {
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            text: { type: 'string' },
            url: { type: 'string' },
            state: { type: 'string', description: 'Playwright 可用：visible、attached、load、domcontentloaded' },
    ...
    },
    {
      ...
      function: {
        ...
        description:
          '一次性观察页面：返回当前 URL、标题、文本摘要、可交互元素 refs、viewport 与截图。复杂网页任务优先用它替代 snapshot+screenshot。',
        parameters: {
          type: 'object',
          properties: {
  ...
  function isWeakToolCallerModel(modelId) {
    ...
    return /mimo|deepseek|deep-seek|qwen2\.?5-?7b|7b-instruct|1\.8b|small/i.test(m);
  }
