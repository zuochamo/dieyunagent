...
function buildAgentStateSnapshot({ reply, trace, plan, results, review, status }) {
  ...
  const failedSubtasks = resultList
    ...
    .filter(Boolean)
    .slice(0, 5);
  // 不能再拿 completedSubtasks.length 当偏移量：它作用在「已过滤的待办」上，
  // 会把还没做的 TODO 直接切掉
